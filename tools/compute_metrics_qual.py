...
def main() -> int:
    ...
    parser.add_argument(
        "--no-fp-penalty-classes",
        type=str,
        default="dorogi,pashni",
        help="Список классов через запятую, для которых не штрафуем за излишнюю разметку (по умолчанию: dorogi,pashni)",
    )
    ...
    # Парсинг списка классов без штрафа за FP
    no_fp_penalty_classes = [cls.strip() for cls in args.no_fp_penalty_classes.split(",") if cls.strip()] if args.no_fp_penalty_classes else []
    ...
    final_score = calculator.calculate_final_f2(
        ...
        no_fp_penalty_classes=no_fp_penalty_classes,
    )
